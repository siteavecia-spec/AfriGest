...
router.put('/:messageId/read', requireAuth, requirePermission('messaging', 'read'), async (req, res) => {
  ...
  try {
    ...
    // Audit best-effort
    try { await auditReq(req, { action: 'messaging.message.read', resource: messageId, userId: auth.sub }) } catch {}
