...
export type OrderItemLike = { sku: string; quantity: number }

async function ensureBoutique(prisma: PrismaClient, preferredId?: string) {
  if (preferredId) {
  ...
  if (!first) throw new Error('No boutique configured')
  return first
}

async function getProductsBySku(prisma: PrismaClient, skus: string[]) {
  const rows = await prisma.product.findMany({ where: { sku: { in: skus } }, select: { id: true, sku: true } })
  const map = new Map<string, string>()
  for (const r of rows) map.set(r.sku, r.id)
  return map
}

export async function reserveSharedStock(prisma: PrismaClient, params: { orderId: string; items: OrderItemLike[]; boutiqueId?: string; actorUserId?: string }) {
  ...
  await prisma.$transaction(async (tx: PrismaClient) => {
    for (const it of params.items) {
      ...
      if (!productId) throw new Error(`SKU not found: ${it.sku}`)
      ...
      await tx.stockAudit.create({ data: { boutiqueId: boutique.id, productId, delta: -Number(it.quantity || 0), reason: `ecom_prepare:${params.orderId}`, userId: params.actorUserId || null } as any })
    }
    await tx.auditLog.create({ data: { action: 'inventory.reserve.shared', resourceId: params.orderId, metadata: { count: params.items.length }, createdAt: new Date() } as any })
  })
  return { ok: true }
}

export async function releaseSharedStock(prisma: PrismaClient, params: { orderId: string; items: OrderItemLike[]; boutiqueId?: string; actorUserId?: string }) {
  const boutique = await ensureBoutique(prisma, params.boutiqueId)
  const skus = Array.from(new Set((params.items || []).map(i => i.sku)))
  const bySku = await getProductsBySku(prisma, skus)
  ...
  await prisma.$transaction(async (tx: PrismaClient) => {
    for (const it of params.items) {
      ...
      await tx.stockAudit.create({ data: { boutiqueId: boutique.id, productId, delta: +Number(it.quantity || 0), reason: `ecom_return:${params.orderId}`, userId: params.actorUserId || null } as any })
    }
    await tx.auditLog.create({ data: { action: 'inventory.release.shared', resourceId: params.orderId, metadata: { count: params.items.length }, createdAt: new Date() } as any })
  })
  return { ok: true }
}
