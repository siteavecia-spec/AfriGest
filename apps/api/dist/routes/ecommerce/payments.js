...
const payloadSchema = zod_1.z.object({
    customer: zod_1.z.object({ email: zod_1.z.string().email().optional(), phone: zod_1.z.string().optional() }).optional(),
...
router.post('/stripe/intent', async (req, res) => {
    ...
    if (!secret)
        return res.status(501).json({ error: 'Stripe not configured', code: 'NOT_CONFIGURED' });
    const parsed = payloadSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid payload' });
    try {
...
router.post('/paypal/order', async (req, res) => {
    ...
    if (!enabled)
        return res.status(501).json({ error: 'PayPal not configured', code: 'NOT_CONFIGURED' });
    const schema = zod_1.z.object({
        items: zod_1.z.array(zod_1.z.object({ sku: zod_1.z.string(), quantity: zod_1.z.number().int().positive(), price: zod_1.z.number().nonnegative(), currency: zod_1.z.string().default('GNF') })).optional(),
    ...
    try {
        ...
        const j = await r.json();
        if (!r.ok)
            return res.status(502).json({ error: 'PayPal create order failed', details: j });
        const approve = (Array.isArray(j.links) ? j.links.find((l) => l.rel === 'approve') : null)?.href;
...
router.post('/paypal/capture', async (req, res) => {
    ...
    if (!enabled)
        return res.status(501).json({ error: 'PayPal not configured', code: 'NOT_CONFIGURED' });
    const schema = zod_1.z.object({ paypalOrderId: zod_1.z.string(), orderId: zod_1.z.string().optional() });
    const parsed = schema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid payload' });
    try {
        ...
        const j = await r.json();
        if (!r.ok)
            return res.status(502).json({ error: 'PayPal capture failed', details: j });
...
router.post('/mtn/init', async (req, res) => {
    ...
    if (!enabled)
        return res.status(501).json({ error: 'MTN MoMo not configured', code: 'NOT_CONFIGURED' });
    const schema = zod_1.z.object({
        items: zod_1.z.array(zod_1.z.object({ sku: zod_1.z.string(), quantity: zod_1.z.number().int().positive(), price: zod_1.z.number().nonnegative(), currency: zod_1.z.string().default('GNF') })).optional(),
        amount: zod_1.z.number().nonnegative().optional(),
        currency: zod_1.z.string().optional(),
...
router.post('/orange/init', async (req, res) => {
    ...
    if (!enabled)
        return res.status(501).json({ error: 'Orange MoMo not configured', code: 'NOT_CONFIGURED' });
    const schema = zod_1.z.object({
        items: zod_1.z.array(zod_1.z.object({ sku: zod_1.z.string(), quantity: zod_1.z.number().int().positive(), price: zod_1.z.number().nonnegative(), currency: zod_1.z.string().default('GNF') })).optional(),
        amount: zod_1.z.number().nonnegative().optional(),
        currency: zod_1.z.string().optional(),
